      ...
      function isVerticalWin() {
        ...
        for (let column = 0; column < columns; column++) {
          for (let row = 0; row <= rows - 4; row++) {
            ...
            let counter = 1;
            for (let i = 1; i < 4; i++) {
              if (board[getIndex(row + i, column)] === board[start]) {
                counter++;
                if (counter === 4)
...
<h1 className="center-container">Welcome to the game, {props.username}!</h1>
    ...
    {currentPlayer === boardSettings.colors.p1 ? `${props.username}'s move` : "Player 2's move"}
      ...
      {win && (
        <>
                ...
                <br></br><br></br><br></br><br></br><br></br>
          <h1 className="center-container" style={{ color: win.winner }}>
            {" "}
